...
// .wrangler/tmp/bundle-Lkwarv/checked-fetch.js
...
// .wrangler/tmp/bundle-Lkwarv/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-Lkwarv/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
function generateSlug(text) {
  return text.toString().toLowerCase().trim().replace(/\s+/g, "-").replace(/[^\w\-가-힣]+/g, "").replace(/\-\-+/g, "-").substring(0, 50);
}
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/admin/api/suggest" && request.method === "POST") {
        ...
        switch (type) {
          case "title":
            prompt = `Keyword: ${keyword}. Suggest one powerful SEO title in Korean.`;
            break;
          case "slug":
            prompt = `Keyword: ${keyword}. Convert to short URL slug (English).`;
            break;
          case "keywords":
            prompt = `Keyword: ${keyword}. List 10 sub-keywords (comma separated).`;
            break;
          case "source":
            prompt = `Keyword: ${keyword}. Suggest one medical source. JSON: {"name": "", "url": ""}.`;
            break;
          case "question":
            prompt = `Keyword: ${keyword}. Suggest a natural user question.`;
            break;
        }
...
async function generateContentHandler(request, env) {
  ...
  const log = /* @__PURE__ */ __name(async (msg) => {
...
`));
  ...
  const task = (async () => {
    try {
      ...
      await log(`\u{1F680} AI \uD504\uB85C\uC138\uC2A4 \uAC00\uB3D9: ${keyword}`);
      ...
      const [textRes, imgRes] = await Promise.all([
        aiCall(universalPrompt, env).then((r) => {
          log(`\u2705 \uD14D\uC2A4\uD2B8 \uC0DD\uC131 \uC804\uC1A1 \uC644\uB8CC`);
          return r;
        }),
        (async () => {
          if (isSEO) {
            log(`\u{1F3A8} \uC774\uBBF8\uC9C0 4\uAC1C \uBCD1\uB82C \uC0DD\uC131 \uC911...`);
            return Promise.all([1, 2, 3, 4].map((i) => env.AI.run(imgModel, { prompt: `${keyword} premium photography, ${selectedStyle}` }).then((r) => {
              log(`\u{1F5BC}\uFE0F \uC774\uBBF8\uC9C0 ${i} \uC644\uB8CC`);
              return r;
            })));
          }
          log(`\u{1F3A8} AEO \uC778\uD3EC\uADF8\uB798\uD53D \uC0DD\uC131 \uC911...`);
